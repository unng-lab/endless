...
func main() {
	...
	flag.Int64Var(&config.MaxTicksPerEpisode, "max-ticks", 1200, "maximum simulation ticks per episode")
	flag.Int64Var(&config.Seed, "seed", time.Now().UnixNano(), "seed for deterministic duel generation")
	flag.IntVar(&config.WorldColumns, "world-columns", 256, "world column count for duel episodes")
	flag.IntVar(&config.WorldRows, "world-rows", 256, "world row count for duel episodes")
